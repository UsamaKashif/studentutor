...
class AboutAcademyForm(forms.Form):
...
class AcademySignUpForm (UserCreationForm):
    ...
    class Meta:
        model = User
        fields = ['username',"email",'city','phone' , "address" ,'name' , "password1", 'password2']
...
class AcademyProfile (ModelForm):
    class Meta:
        model = Academy
        fields = "__all__"
        exclude = ['academy', "email", "username"]


from students.validators import validate_file_extension
